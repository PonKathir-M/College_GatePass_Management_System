...
exports.analytics = async (req, res, next) => {
  try {
    ...
    const approved = await GatePass.count({ where: { status: ["HOD Approved", "Approved", "Completed"] } });
...
exports.getDashboardStats = async (req, res, next) => {
  try {
    ...
    const weeklyApproved = await GatePass.count({
      where: {
        createdAt: { [Op.gte]: thisWeek },
        status: ["Approved", "HOD Approved", "Completed"]
      }
    });
    ...
    const weeklyPending = await GatePass.count({
      where: {
        createdAt: { [Op.gte]: thisWeek },
        status: { [Op.notIn]: ["Approved", "HOD Approved", "Completed", "Rejected", "Cancelled"] }
      }
    });
...
exports.getAdvancedStats = async (req, res, next) => {
  try {
    ...
    const summary = {
      total: passes.length,
      approved: passes.filter(p => ["HOD Approved", "Approved", "Completed"].includes(p.status)).length,
