  ...
  const fetchHistory = async () => {
    try {
      ...
      const url = apiEndpoint
        ...
        : `http://localhost:5001/api/hod/student-history/${studentId}`;
          ...
          <>
            ...
            <div className="popup-profile-wrapper">
              {historyData.student?.profile_pic ? (
                <img src={getImageSrc(historyData.student.profile_pic)} alt="Student" className="popup-profile-img" />
              ) : (
                <div className="popup-profile-img" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fff', fontSize: '2rem' }}>S</div>
              )}
            </div>
                ...
                <div className="history-list">
                  {historyData.history?.map((pass) => (
                    <div key={pass.gatepass_id} className={`history-item ${pass.status.toLowerCase().replace(' ', '-')}`}>
                      ...
                      <div className="h-reason">Reason: {pass.reason}</div>
                      {pass.rejection_reason && (
                        <div className="h-rejection-msg">Rejection Reason: {pass.rejection_reason}</div>
                      )}
                    </div>
                  ))}
